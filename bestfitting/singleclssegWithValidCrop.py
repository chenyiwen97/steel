...
from lossfuncs.loss import calc_loss
...
from datasets.datastes import ImageDataSingleClsWithValidCrop, MyBalanceClassSampler
from myutils.cal_util import AverageMeter, metric
from myutils.augumentation_util import dataAugumentationSingleCls, dataAugumentationAfterCrop
...
def main(cls_label):
    ...
    train_csv = train_csv[(pd.isnull(train_csv[cls_label]).astype(np.int32) == 0)]
    ...
    train_data, val_data = train_test_split(train_csv, test_size = 0.2, random_state=69)

    train_dataset = ImageDataSingleClsWithValidCrop(train_data, cls_label ,path, dataAugumentationAfterCrop, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), subset="train")
    train_loader = DataLoader(dataset=train_dataset, batch_size=set_batch_size, shuffle=True, pin_memory=True)
    ...
    valid_dataset = ImageDataSingleClsWithValidCrop(val_data, cls_label, path, dataAugumentationAfterCrop, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), subset="valid")
    valid_loader = DataLoader(dataset=valid_dataset, batch_size=set_batch_size, shuffle=True, pin_memory=True)
...
def save_model(model, model_out_dir, epoch, model_name,
    ...
    model_fpath = os.path.join(model_out_dir, '%s.pth' % model_name)
    torch.save({
    ...
    optim_fpath = os.path.join(model_out_dir, '%s_optim.pth' % model_name)
...
model_out_dir = ""
# model_out_dir = "./model/1009fordefect_" + str(cls_label)
...
if __name__ == '__main__':
    ...
    for cls_label in [3,2,1,4]:
        model_out_dir = "E:/george/model/10112058URfordefect_" + str(cls_label)
