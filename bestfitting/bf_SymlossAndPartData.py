...
def main():
    ...
    train_dataset = ImageData(train_data, path, dataAugumentation, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), subset="train")
    # train_loader = DataLoader(dataset=train_dataset, batch_size=8, shuffle=True)
    myTrainSample = MyBalanceClassSampler(train_dataset)
    train_loader = DataLoader(dataset=train_dataset, batch_size=1, sampler=myTrainSample, pin_memory=True)

    valid_dataset = ImageData(val_data, path, augmentation = dataAugumentation, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), subset="train")
    # valid_loader = DataLoader(dataset=valid_dataset, batch_size=8, shuffle=True)
    myValSample = MyBalanceClassSampler(valid_dataset)
    valid_loader = DataLoader(dataset=valid_dataset, batch_size=1, sampler=myValSample, pin_memory=True)
...
def myplot(scores, name):
    ...
    plt.legend()
    # plt.show()
    plt.savefig(os.path.join(model_out_dir, name) + ".png")
...
model_out_dir = "./model/0927USRSymLossWithPartData"
...
# torch.save(net.state_dict(), "./model_bceabdnormalization2.pth")
"""
