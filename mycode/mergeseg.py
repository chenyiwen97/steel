...
for index in range(0, len(test_data),4):
    ...
    img = img.cuda()
    output = torch.softmax(model(img),1).data.cpu()
    # for i in range(5):
    #     print(output[0,i,0,0])
    output2 = torch.softmax(model2(img),1).data.cpu()
    # output3 = model3(img).data.cpu()
...
test_data.to_csv('E:/pycharm_project/steel/code/submit/mergeseg.csv',index=False)