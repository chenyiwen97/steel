...
def make_mask(row_id, df):
    ...
    masks = np.zeros((768, 768, 5), dtype=np.float32) # float32 is V.Imp
    # 4:class 1～4 (ch:0～3)
    ...
    for idx, label in enumerate(labels.values):
        if not pd.isnull(label):
            ...
            masks[:, :, idx] = mask.reshape(768, 768, order='F')
    logi=masks[:,:,0]
    for ie in range(1,4):
...
class SteelDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_path = os.path.join(self.root, "resize_image", image_id)
    ...
    def start(self):
        for epoch in range(self.num_epochs):
            ...
            if val_loss < self.best_loss:
                ...
                torch.save(self.net.state_dict(), "./unetb0resize.pth")
